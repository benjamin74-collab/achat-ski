    ...
    <div className="container-page py-10">
      ...
      <section className="mt-8">
        ...
        <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            <a key={c.href} href={c.href} className="card p-4 hover:bg-neutral-50">{c.label}</a>
          ))}
        </div>
      </section>
      ...
      <section className="mt-10">
        ...
        <ul className="mt-3 list-disc pl-6">
          {latest.map(p => (
            <li key={p.id}><a className="underline" href={`/p/${p.slug}`}>{p.brand} {p.model} {p.season}</a></li>
          ))}
        </ul>
      </section>
    </div>
